...
class WP2Static_BunnyCDN extends WP2Static_SitePublisher
{
    public function __construct()
    {
        ...
        $this->previous_hashes_path = $this->settings['wp_uploads_path'] . '/WP2STATIC-BUNNYCDN-PREVIOUS-HASHES.txt';
        if (defined('WP_CLI')) {
            return;
        }
    ...
    public function upload_files()
    {
        ...
        foreach ($lines as $line) {
            ...
            if (isset($this->settings['bunnycdnRemotePath'])) {
                $this->target_path = $this->settings['bunnycdnRemotePath'] . '/' . $this->target_path;
            }
    ...
    {
        try {
            $endpoint = 'https://bunnycdn.com/api/pullzone/' . $this->settings['bunnycdnPullZoneID'] . '/purgeCache';
            $ch = curl_init();
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json', 'Content-Length: 0', 'AccessKey: ' . $this->settings['bunnycdnPullZoneAccessKey'],));
            $output = curl_exec($ch);
    ...
    {
        try {
            $remote_path = $this->api_base . '/' . $this->settings['bunnycdnStorageZoneName'] . '/tmpFile';
            $ch = curl_init();
            ...
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('AccessKey: ' . $this->settings['bunnycdnStorageZoneAccessKey'],));
            $post_options = array('body' => 'Test WP2Static connectivity',);
    ...
    {
        try {
            $remote_path = $this->api_base . '/' . $this->settings['bunnycdnStorageZoneName'] . '/' . $this->target_path;
            $headers = array('AccessKey: ' . $this->settings['bunnycdnStorageZoneAccessKey'],);
            $this->client->putWithFileStreamAndHeaders($remote_path, $this->local_file, $headers);
