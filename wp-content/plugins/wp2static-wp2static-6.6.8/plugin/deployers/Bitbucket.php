...
class WP2Static_BitBucket extends WP2Static_SitePublisher
{
    public function __construct()
    {
        ...
        list($this->user, $this->repository) = explode('/', $this->settings['bbRepo']);
        $this->api_base = 'https://api.bitbucket.org/2.0/repositories/';
        $this->previous_hashes_path = $this->settings['wp_uploads_path'] . '/WP2STATIC-BITBUCKET-PREVIOUS-HASHES.txt';
        if (defined('WP_CLI')) {
            return;
        }
    ...
    public function test_upload()
    {
        ...
        try {
            ...
            $post_options = array('.tmp_wp2static.txt' => 'Test WP2Static connectivity', '.tmp_wp2static.txt' => 'Test WP2Static connectivity #2', 'message' => 'WP2Static deployment test',);
            $this->client->postWithArray($remote_path, $post_options, $curl_options = array(CURLOPT_USERPWD => $this->user . ':' . $this->settings['bbToken'],));
    ...
    public function addFileToBatchForCommitting($line)
    {
        ...
        if (isset($this->settings['bbPath'])) {
            $this->target_path = $this->settings['bbPath'] . '/' . $this->target_path;
        }
        ...
        if (isset($this->file_paths_and_hashes[$this->target_path])) {
            ...
            if ($prev != $current) {
                $this->files_data['/' . rtrim($this->target_path)] = new CURLFile($local_file);
