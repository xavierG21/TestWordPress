...
class WP2Static_SitePublisher
{
    ...
    {
        $target_settings = array('general', 'wpenv', 'advanced',);
    ...
    {
        $this->export_file_list = $this->settings['wp_uploads_path'] . '/WP2STATIC-FILES-TO-DEPLOY.txt';
        $this->archive_dir = file_get_contents($this->settings['wp_uploads_path'] . '/WP2STATIC-CURRENT-ARCHIVE.txt');
    }
    public function pauseBetweenAPICalls()
    {
        if (isset($this->settings['delayBetweenAPICalls']) && $this->settings['delayBetweenAPICalls'] > 0) {
    ...
    {
        $this->batch_index++;
        $completed_urls = $this->total_urls_to_crawl - $this->files_remaining + $this->batch_index;
        require_once dirname(__FILE__) . '/ProgressLog.php';
        ProgressLog::l($completed_urls, $this->total_urls_to_crawl);
    }
    public function initiateProgressIndicator()
    {
        $this->deploy_count_path = $this->settings['wp_uploads_path'] . '/WP-STATIC-TOTAL-FILES-TO-DEPLOY.txt';
        $this->total_urls_to_crawl = file_get_contents($this->deploy_count_path);
        $this->batch_index = 0;
    }
    ...
    {
        $original_filepath = str_replace('\\', '\\\\', $file_in_archive);
        $original_file_without_archive = str_replace($replace_path, '', $original_filepath);
        $original_file_without_archive = ltrim($original_file_without_archive, '/');
    ...
    public function getArchivePathForReplacement($archive_path)
    {
        ...
        $local_path_to_strip = str_replace('//', '/', $local_path_to_strip);
        return $local_path_to_strip;
    }
    public function getRemoteDeploymentPath($dir, $file_in_archive, $archive_path_to_replace, $basename_in_target)
    {
        $deploy_path = str_replace($archive_path_to_replace, '', $dir);
        $deploy_path = ltrim($deploy_path, '/');
        $deploy_path .= '/';
        if ($basename_in_target) {
            $deploy_path .= basename($file_in_archive);
        }
        $deploy_path = ltrim($deploy_path, '/');
        return $deploy_path;
    }
    ...
    {
        $archive_path_to_replace = $this->getArchivePathForReplacement($this->archive->path);
        foreach (scandir($dir) as $item) {
            ...
            if (is_dir($file_in_archive)) {
                $this->createDeploymentList($file_in_archive, $basename_in_target);
            } elseif (is_file($file_in_archive)) {
                $local_file_path = $this->getLocalFileToDeploy($file_in_archive, $archive_path_to_replace);
                $remote_deployment_path = $this->getRemoteDeploymentPath($dir, $file_in_archive, $archive_path_to_replace, $basename_in_target);
                file_put_contents($this->export_file_list, $local_file_path . ',' . $remote_deployment_path . PHP_EOL, FILE_APPEND | LOCK_EX);
    ...
    {
        $this->clearFileList();
        $this->createDeploymentList($this->settings['wp_uploads_path'] . '/' . $this->archive->name, $basename_in_target);
        $linecount = 0;
        ...
        fclose($handle);
        $deploy_count_path = $this->settings['wp_uploads_path'] . '/WP-STATIC-TOTAL-FILES-TO-DEPLOY.txt';
        file_put_contents($deploy_count_path, $linecount, LOCK_EX);
    ...
    public function getItemsToDeploy($batch_size = 1)
    {
        ...
        file_put_contents($this->export_file_list, implode(PHP_EOL, $contents));
        chmod($this->export_file_list, 0664);
        return $lines;
    }
    ...
    public function checkForValidResponses($code, $good_codes)
    {
        if (!in_array($code, $good_codes)) {
            ...
            throw new Exception('BAD RESPONSE STATUS FROM API (' . $code . ')');
        }
    }
    ...
    public function recordFilePathAndHashInMemory($target_path, $local_file_contents)
    {
        $this->file_paths_and_hashes[$target_path] = crc32($local_file_contents);
    }
