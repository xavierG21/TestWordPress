...
class CSSProcessor extends WP2Static
{
    ...
    public function processCSS($css_document, $page_url)
    {
        ...
        foreach ($this->css_doc->getAllValues() as $node_value) {
            if ($node_value instanceof Sabberworm\CSS\Value\URL) {
                ...
                $inline_img = strpos($original_link, 'data:image');
                if ($inline_img !== false) {
                    continue;
                }
                ...
                if ($this->isInternalLink($original_link)) {
                    ...
                    $rewritten_url = str_replace($rewrite_from, $rewrite_to, $original_link);
                    $rewritten_url = new Sabberworm\CSS\Value\CSSString($rewritten_url);
    ...
    {
        $rewritten_source = str_replace(array($this->settings['wp_site_url'], addcslashes($this->settings['wp_site_url'], '/'),), array($this->placeholder_url, addcslashes($this->placeholder_url, '/'),), $this->raw_css);
