...
class WPSite
{
    public function __construct()
    {
        ...
        // WP_CLI::line("--File directories -- \n".implode("; \n", $wp_upload_path_and_url));
        ...
        $this->site_path = wp_normalize_path(get_home_path());
        ...
        $this->wp_inc = '/' . WPINC;
        $this->wp_content = str_replace($this->site_path, '/', $this->wp_content_path);
        $this->wp_uploads = str_replace($this->site_path, '/', $this->wp_uploads_path);
        $this->wp_plugins = str_replace($this->site_path, '/', $this->plugins_path);
        $this->wp_themes = str_replace($this->site_path, '/', $this->theme_root_path);
        ...
        // WP_CLI::line($this->wp_content . " --wp_content");
    }
    ...
    public function getWPDirFullPath($wp_dir)
    {
        ...
        return wp_normalize_path(rtrim($full_path, '/'));
    }
    public function getWPDirNameOnly($wp_dir)
    {
        ...
        switch ($wp_dir) {
            ...
            case 'theme-root':
            case 'plugins':
                $wp_dir_name = $this->getLastPathSegment($this->getWPDirFullPath($wp_dir));
                break;
        }
