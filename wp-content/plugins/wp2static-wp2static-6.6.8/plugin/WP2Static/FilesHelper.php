...
class WP2Static_FilesHelper
{
    ...
    public static function getThemeFiles($theme_type)
    {
        ...
        if (is_dir($directory)) {
            ...
            foreach ($iterator as $filename => $file_object) {
                ...
                $detected_filename = str_replace($template_path, $template_url, $filename);
    ...
    public static function detectVendorFiles($wp_site_url)
    {
        ...
        if (class_exists('\\Elementor\Api')) {
            ...
            $elementor_urls = self::getListOfLocalFilesByUrl($elementor_font_dir);
            $vendor_files = array_merge($vendor_files, $elementor_urls);
        }
        if (defined('WPSEO_VERSION')) {
            $yoast_sitemaps = array('/sitemap_index.xml', '/post-sitemap.xml', '/page-sitemap.xml', '/category-sitemap.xml', '/author-sitemap.xml',);
            $vendor_files = array_merge($vendor_files, $yoast_sitemaps);
        }
        if (is_dir(WP_PLUGIN_DIR . '/soliloquy/')) {
            $soliloquy_assets = WP_PLUGIN_DIR . '/soliloquy/assets/css/images/';
            $soliloquy_urls = self::getListOfLocalFilesByUrl($soliloquy_assets);
        ...
        if (class_exists('autoptimizeMain')) {
            ...
            $prefix = str_replace($wp_site->site_url, '/', $wp_site->wp_content_url);
            $autoptimize_urls = self::getAutoptimizeCacheFiles($autoptimize_cache_dir, $wp_site->wp_content_path, $prefix);
        ...
        if (class_exists('Custom_Permalinks')) {
            ...
            $posts = $wpdb->get_results(sprintf($query, $wpdb->postmeta, 'custom_permalink'));
            if ($posts) {
        ...
        if (class_exists('molongui_authorship')) {
            ...
            $molongui_urls = self::getListOfLocalFilesByUrl($molongui_path);
    ...
    public static function recursively_scan_dir($dir, $siteroot, $list_path)
    {
        ...
        foreach ($files as $item) {
            if ($item != '.' && $item != '..' && $item != '.git') {
                if (is_dir($dir . '/' . $item)) {
                    self::recursively_scan_dir($dir . '/' . $item, $siteroot, $list_path);
                    ...
                    file_put_contents($list_path, $filename, FILE_APPEND | LOCK_EX);
                    chmod($list_path, 0664);
                }
            }
        }
    }
    public static function getAutoptimizeCacheFiles($cache_dir, $path_to_trim, $prefix)
    {
        $files = array();
        $directory = $cache_dir;
        if (is_dir($directory)) {
            ...
            foreach ($iterator as $filename => $file_object) {
                $path_crawlable = self::filePathLooksCrawlable($filename);
                if ($path_crawlable) {
                    array_push($files, $prefix . home_url(str_replace($path_to_trim, '', $filename)));
                }
            }
        }
        return $files;
    ...
    public static function filePathLooksCrawlable($file_name)
    {
        ...
        $filenames_to_ignore = array('.DS_Store', '.PHP', '.SQL', '.git', '.idea', '.ini', '.map', '.php', '.sql', '.yarn', 'WP-STATIC', '__MACOSX', 'backwpup', 'bower.json', 'bower_components', 'composer.json', 'current-export', 'gulpfile.js', 'latest-export', 'node_modules', 'package.json', 'pb_backupbuddy', 'previous-export', 'thumbs.db', 'vendor', 'wp-static-html-output',);
        ...
        $extensions_to_ignore = array('php', 'phtml', 'tpl', 'less', 'scss', 'po', 'mo', 'tar.gz', 'zip', 'txt', 'po', 'pot', 'sh', 'sh', 'mo', 'md',);
        if (in_array($path_info['extension'], $extensions_to_ignore)) {
            return false;
        }
        return true;
    }
    public static function buildInitialFileList($via_cli = false, $uploads_path, $uploads_url, $settings)
    {
        require_once dirname(__FILE__) . '/WPSite.php';
        $wp_site = new WPSite();
        $base_url = untrailingslashit(home_url());
        $url_queue = array_merge(array(trailingslashit($base_url)), array('/robots.txt'), array('/favicon.ico'), array('/sitemap.xml'));
        switch ($settings['detection_level']) {
            case 'homepage':
                break;
            case 'posts_and_pages':
                $url_queue = array_merge($url_queue, self::getAllWPPostURLs($base_url));
                break;
            case 'everything':
            default:
                $url_queue = array_merge($url_queue, self::getThemeFiles('parent'), self::getThemeFiles('child'), self::detectVendorFiles($wp_site->site_url), self::getListOfLocalFilesByUrl($uploads_url), self::getAllWPPostURLs($base_url));
        }
        $url_queue = self::cleanDetectedURLs($url_queue);
        $url_queue = apply_filters('wp2static_modify_initial_crawl_list', $url_queue);
        $unique_urls = array_unique($url_queue);
        sort($unique_urls);
        $initial_crawl_list_total = count($unique_urls);
        $str = implode("\n", $unique_urls);
        file_put_contents($uploads_path . '/WP-STATIC-INITIAL-CRAWL-LIST.txt', $str);
        chmod($uploads_path . '/WP-STATIC-INITIAL-CRAWL-LIST.txt', 0664);
        file_put_contents($uploads_path . '/WP-STATIC-INITIAL-CRAWL-TOTAL.txt', $initial_crawl_list_total);
    ...
    public static function getAllWPPostURLs($wp_site_url)
    {
        ...
        $posts = $wpdb->get_results(sprintf($query, $wpdb->posts, 'publish', 'revision', 'nav_menu_item'));
        foreach ($posts as $post) {
        ...
        foreach ($taxonomies as $taxonomy) {
            ...
            foreach ($terms as $term) {
                ...
                $term_url = str_replace($wp_site_url, '', $permalink);
                $category_links[$term_url] = $total_posts;
                $post_urls[] = $permalink;
            }
        }
        $category_pagination_urls = self::getPaginationURLsForCategories($category_links);
        $post_pagination_urls = self::getPaginationURLsForPosts(array_unique($unique_post_types));
        $comment_pagination_urls = self::getPaginationURLsForComments($wp_site_url);
        $post_urls = array_merge($post_urls, $post_pagination_urls, $category_pagination_urls, $comment_pagination_urls);
        return $post_urls;
    }
    public static function cleanDetectedURLs($urls)
    {
        ...
        $search_text = ' ';
        $url_queue = array_filter($unique_urls, 'filter_arr_empty_vals');
        $stripped_urls = str_replace($wp_site_url, '/', $url_queue);
        $cleaned_urls = str_replace('//', '/', $stripped_urls);
        return $cleaned_urls;
    }
    public static function getPaginationURLsForPosts($post_types)
    {
        ...
        foreach ($post_types as $post_type) {
            $query = "
                ...
                WHERE post_status = '%s'
                AND post_type = '%s'";
            $count = $wpdb->get_results(sprintf($query, $wpdb->posts, 'publish', $post_type));
            $post_type_obj = get_post_type_object($post_type);
            $plural_form = strtolower($post_type_obj->labels->name);
            $count = $wpdb->num_rows;
            $total_pages = ceil($count / $default_posts_per_page);
            for ($page = 1; $page <= $total_pages; $page++) {
                $pagination_url = "/{$plural_form}/{$pagination_base}/{$page}";
                $urls_to_include[] = str_replace('/posts/', '/', $pagination_url);
            }
        }
    ...
    public static function getPaginationURLsForComments($wp_site_url)
    {
        ...
        foreach (get_comments() as $comment) {
            ...
            $urls_to_include[] = str_replace($wp_site_url, '', $comment_url);
        }
