...
class ArchiveProcessor extends WP2Static
{
    public function __construct()
    {
        ...
        $this->loadSettings(array('wpenv', 'crawling', 'advanced', 'processing', 'netlify', 'zip', 'folder',));
    }
    public function renameWPDirectory($source, $target)
    {
        if (empty($source) || empty($target)) {
            require_once dirname(__FILE__) . '/../WP2Static/WsLog.php';
            WsLog::l('Failed trying to rename: ' . 'Source: ' . $source . ' to: ' . $target);
            die();
        }
        ...
        if (is_dir($original_dir)) {
            ...
            WP2Static_FilesHelper::delete_dir_with_files($original_dir);
        } else {
            require_once dirname(__FILE__) . '/../WP2Static/WsLog.php';
            WsLog::l('Trying to rename non-existent directory: ' . $original_dir);
        }
    }
    ...
    public function copyStaticSiteToPublicFolder()
    {
        if ($this->settings['selected_deployment_option'] === 'folder') {
            ...
            if ($directory_exists) {
                ...
                if (wp_mkdir_p($target_folder)) {
                    if (!$this->put_safety_file($target_folder)) {
                        require_once dirname(__FILE__) . '/../WP2Static/WsLog.php';
                        WsLog::l('Couldn\'t put safety file in ' . 'Target Directory' . $target_folder);
                        die();
                    }
                } else {
                    require_once dirname(__FILE__) . '/../WP2Static/WsLog.php';
                    WsLog::l('Couldn\'t create Target Directory: ' . $target_folder);
                    die();
                }
            }
            if ($directory_empty) {
            ...
            $dir_has_safety_file = $this->dir_has_safety_file($target_folder);
            if ($directory_empty || $dir_has_safety_file) {
                $this->recursive_copy($this->archive->path, $this->target_folder);
                if (!$this->put_safety_file($target_folder)) {
                    require_once dirname(__FILE__) . '/../WP2Static/WsLog.php';
                    WsLog::l('Couldn\'t put safety file in ' . 'Target Directory' . $target_folder);
                    die();
                ...
                require_once dirname(__FILE__) . '/../WP2Static/WsLog.php';
                WsLog::l('Target Directory wasn\'t empty ' . 'or didn\'t contain safety file ' . $target_folder);
                die();
            }
        }
    }
    ...
    public function create_zip()
    {
        ...
        foreach ($iterator as $filename => $file_object) {
            ...
            if ($base_name != '.' && $base_name != '..') {
                if (!$zip_archive->addFile(realpath($filename), str_replace($this->archive->path, '', $filename))) {
                    return new WP_Error('Could not add file: ' . $filename);
                }
            }
        }
        $zip_archive->close();
        $zip_path = $this->settings['wp_uploads_path'] . '/' . $this->archive->name . '.zip';
    ...
    public function removeWPCruft()
    {
        ...
        WP2Static_FilesHelper::delete_dir_with_files($this->archive->path . '/wp-json/');
    }
