...
class Net_URL2
{
    ...
    private $_options = array(self::OPTION_STRICT => true, self::OPTION_USE_BRACKETS => true, self::OPTION_DROP_SEQUENCE => true, self::OPTION_ENCODE_KEYS => true, self::OPTION_SEPARATOR_INPUT => '&', self::OPTION_SEPARATOR_OUTPUT => '&',);
    ...
    public function getQueryVariables()
    {
        ...
        $return = array();
        for ($part = strtok($this->_query, $separator); strlen($part); $part = strtok($separator)) {
    ...
    private function _queryArrayByKey($key, $value, array $array = array())
    {
        ...
        if (!$offset) {
            ...
            $array[$name] = $this->_queryArrayByBrackets($brackets, $value, $array[$name]);
        }
        return $array;
    }
    private function _queryArrayByBrackets($buffer, $value, array $array = null)
    {
        ...
        for ($iteration = 0; strlen($buffer); $iteration++) {
            ...
            if ($open !== 0) {
                ...
                throw new Exception('Net_URL2 Internal Error: ' . __METHOD__ . '(): ' . 'Opening bracket [ must exist at offset 0');
            }
            $close = strpos($buffer, ']', 1);
            if (!$close) {
                throw new Exception('Net_URL2 Internal Error: ' . __METHOD__ . '(): ' . 'Closing bracket ] must exist, not found');
            }
    ...
    {
        if (false !== $open = strpos($key, '[') and false === strpos($key, ']', $open + 1)) {
            $open = false;
        }
        return $open;
    }
    public function setQueryVariables(array $array)
    {
        if (!$array) {
            ...
            $this->_query = $this->buildQuery($array, $this->getOption(self::OPTION_SEPARATOR_OUTPUT));
        }
        return $this;
    }
    ...
    {
        return preg_replace_callback('((?:%[0-9a-fA-Z]{2})+)', array($this, '_normalizeCallback'), $mixed);
    }
    ...
    public function resolve($reference)
    {
        ...
        if (!$reference->_isFragmentOnly() && !$this->isAbsolute()) {
            throw new Exception('Base-URL must be absolute if reference is not fragment-only');
        }
    ...
    {
        return ($this->_fragment !== false && $this->_query === false && $this->_path === '' && $this->_port === false && $this->_host === false && $this->_userinfo === false && $this->_scheme === false);
    }
    public static function removeDotSegments($path)
    {
        ...
        if ($path !== '') {
            $message = sprintf('Unable to remove dot segments; hit loop limit %d (left: %s)', $j, var_export($path, true));
    ...
    public static function getCanonical()
    {
        ...
        if ($url->_scheme == 'http' && $port != 80 || $url->_scheme == 'https' && $port != 443) {
            $url->_port = $port;
        }
        return $url;
    }
    ...
    {
        $query = array();
        $drop_names = ($this->_options[self::OPTION_DROP_SEQUENCE] === true && array_keys($data) === array_keys(array_values($data)));
    ...
    protected function parseUrl($url)
    {
        ...
        $this->_query = !empty($matches[6]) ? $this->_encodeData($matches[7]) : false;
        $this->_fragment = !empty($matches[8]) ? $matches[9] : false;
    }
    private function _encodeData($url)
    {
        return preg_replace_callback('([\x-\x20\x22\x3C\x3E\x7F-\xFF]+)', array($this, '_encodeCallback'), $url);
    }
