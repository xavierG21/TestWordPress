...
class WP2Static_Controller
{
    ...
    public static function getInstance()
    {
        if (null === self::$instance) {
            ...
            self::$instance->options = new WP2Static_Options(self::OPTIONS_KEY);
    ...
    public function set_menu_order($menu_order)
    {
        ...
        $order = array('index.php', 'wp2static',);
        return $order;
    }
    public function setDefaultOptions()
    {
        if (null === $this->options->getOption('version')) {
            $this->options->setOption('version', self::VERSION)->setOption('static_export_settings', self::VERSION)->setOption('rewriteWPPaths', '1')->setOption('removeConditionalHeadComments', '1')->setOption('removeWPMeta', '1')->setOption('removeWPLinks', '1')->setOption('removeHTMLComments', '1')->save();
        }
    }
    ...
    public function activate($network_wide)
    {
        if ($network_wide) {
            ...
            $site_ids = $wpdb->get_col(sprintf($query, $wpdb->blogs, $wpdb->siteid));
    ...
    public function registerOptionsPage()
    {
        ...
        $page = add_menu_page(__('WP2Static', 'static-html-output-plugin'), __('WP2Static', 'static-html-output-plugin'), 'manage_options', self::HOOK, array(self::$instance, 'renderOptionsPage'), 'dashicons-shield-alt');
        add_action('admin_print_styles-' . $page, array($this, 'enqueueAdminStyles',));
    }
    public function enqueueAdminStyles()
    {
        $plugins_url = plugin_dir_url(dirname(__FILE__));
        wp_enqueue_style(self::HOOK . '-admin', $plugins_url . 'wp2static.css?sdf=sdfd', null, $this::VERSION);
    }
    ...
    public function generate_filelist_preview()
    {
        ...
        $target_settings = array('general', 'crawling',);
        if (defined('WP_CLI')) {
            require_once dirname(__FILE__) . '/WP2Static/DBSettings.php';
            $this->settings = WPSHO_DBSettings::get($target_settings);
        } else {
            require_once dirname(__FILE__) . '/WP2Static/PostSettings.php';
            $this->settings = WPSHO_PostSettings::get($target_settings);
        }
        $plugin_hook = 'wp2static';
        $initial_file_list_count = WP2Static_FilesHelper::buildInitialFileList(true, $this->wp_site->wp_uploads_path, $this->wp_site->uploads_url, $this->settings);
        if (!defined('WP_CLI')) {
    ...
    public function renderOptionsPage()
    {
        ...
        $this->view->setTemplate('options-page-js')->assign('options', $this->options)->assign('wp_site', $this->wp_site)->assign('onceAction', self::HOOK . '-options')->render();
        $this->view->setTemplate('options-page')->assign('wp_site', $this->wp_site)->assign('options', $this->options)->assign('onceAction', self::HOOK . '-options')->render();
    }
    ...
    {
        require_once dirname(__FILE__) . '/WP2Static/Exporter.php';
    ...
    {
        require_once dirname(__FILE__) . '/WP2Static/ArchiveProcessor.php';
    ...
    public function delete_deploy_cache()
    {
        ...
        $cache_files = array('/WP2STATIC-GITLAB-PREVIOUS-HASHES.txt', '/WP2STATIC-GITHUB-PREVIOUS-HASHES.txt', '/WP2STATIC-S3-PREVIOUS-HASHES.txt', '/WP2STATIC-BUNNYCDN-PREVIOUS-HASHES.txt', '/WP2STATIC-BITBUCKET-PREVIOUS-HASHES.txt', '/WP2STATIC-FTP-PREVIOUS-HASHES.txt',);
